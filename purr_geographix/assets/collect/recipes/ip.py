...
selector = f"""
    ...
    t AS (
        SELECT
            ...
            LIST(IFNULL(run_number,                 '{PURR_NULL}', CAST(run_number AS VARCHAR)),                '{PURR_DELIM}' ORDER BY test_number)  AS t_run_number,
            LIST(IFNULL(shut_off_type,              '{PURR_NULL}', CAST(shut_off_type AS VARCHAR)),             '{PURR_DELIM}' ORDER BY test_number)  AS t_shut_off_type,
            LIST(IFNULL(source,                     '{PURR_NULL}', CAST(source AS VARCHAR)),                    '{PURR_DELIM}' ORDER BY test_number)  AS t_source,
            ...
            LIST(IFNULL(test_type,                  '{PURR_NULL}', CAST(test_type AS VARCHAR)),                 '{PURR_DELIM}' ORDER BY test_number)  AS t_test_type,
            ...
            LIST(IFNULL(uwi,                        '{PURR_NULL}', CAST(uwi AS VARCHAR)),                       '{PURR_DELIM}' ORDER BY test_number)  AS t_uwi,
